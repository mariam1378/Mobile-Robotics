...
class ContainerChaser(Node):
    def __init__(self):
        ...
        self.declare_parameter("desired_midpoint_x", 350)
        self.declare_parameter("desired_midpoint_y", 160)
    ...
    def listener_callback(self, msg):
        cmd_vel_msg = Twist()
        if not ((self.desired_midpoint_x - 1) <= msg.x <= (self.desired_midpoint_x + 1)):
            cmd_vel_msg.angular.z = -(np.sign(msg.x - self.desired_midpoint_x)) * self.max_angular_speed
        else:
            cmd_vel_msg.angular.z = 0.0

        if not ((self.desired_midpoint_y - 1) <= msg.y <= (self.desired_midpoint_y + 1)):
